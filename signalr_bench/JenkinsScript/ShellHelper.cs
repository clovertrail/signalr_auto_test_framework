...
namespace JenkinsScript
{
    class ShellHelper
    {
        ...
        public static (int, string) GenerateAllReports(List<string> hosts, AgentConfig agentConfig, string serviceType, string transportType, string hubProtocol, string scenario)
        {
            var errCode = 0;
            var result = "";
            var cmd = "";

            cmd = $"cd /home/{agentConfig.User}/signalr-bench/; export bench_type_list='{serviceType}'; export bench_codec_list='{hubProtocol}'; export bench_name_list='{scenario}'; sh gen_all_report.sh; sh publish_report.sh; sh gen_summary.sh;";
            Util.Log($"CMD: {agentConfig.User}@{agentConfig.Master}: {cmd}");
            (errCode, result) = ShellHelper.RemoteBash(agentConfig.User, agentConfig.Master, agentConfig.SshPort, agentConfig.Password, cmd);
